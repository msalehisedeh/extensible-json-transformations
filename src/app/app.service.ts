...
export class AppService {
    ...
    private users = [
      ...
      private usersTrans = {
        ...
        "importUrls": [],
        "rootTemplate": ".",
        "onResult": "filter(eyeColor,=,blue)",
        "templates": [{
          "name": ".",
          "match": "",
          "context": "root",
          "style": {
            "name": "join(split(valueOf(name), ' '), ', ')",
            "isActive": "valueOf(isActive)",
            "balance": "valueOf(balance)",
            "eyeColor": "valueOf(eyeColor)",
            "friends": "offPool(friends,valueOf(friends.id))",
            "location": "concat(valueOf(latitude),',',valueOf(longitude))",
            "address": "join(enlist(valueOf(address.street),valueOf(address.city),valueOf(address.zipcode)),',')"
          }
        },
        {
          "name": "friends",
    ...
    usersList() {
        ...
        return Observable.of({data: this.users, transformations: this.usersTrans});
    }
    eventsList() {
      // 'http://acesinc.net/introducing-a-streaming-json-data-generator/'
      return Observable.of({data: this.events, transformations: this.eventsTrans});
  }
  productsList() {
    // 'http://acesinc.net/introducing-a-streaming-json-data-generator/'
    return Observable.of({data: this.products, transformations: this.productsTrans});
}
