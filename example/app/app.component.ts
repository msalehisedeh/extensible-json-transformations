...
export class AppComponent {
  ...
  descriptions: any = {
    ...
    apply:
      'Apply the template name in current context for each node that matches the given path',
  ...
  constructor(private service: AppService) {
    this.service.usersList().subscribe((results) => {
      ...
      this.transformationsCopy = JSON.parse(
        JSON.stringify(this.data.transformations)
      );
    });
  }
  ...
  addDataEntry(entryName: string, entryJson: any, transJson: any) {
    if (entryName.length && entryJson.length) {
      ...
      const trans = this.toJson(
        transJson,
        'We are unable to validate JSON transformations. Please clear text and try again!'
      );
      if (entry && trans) {
        ...
        this.data.transformations =
          this.selectionContents[entryName].transformations;
        this.transformationsCopy = JSON.parse(
          JSON.stringify(this.data.transformations)
        );
      }
    ...
    if (code === 13) {
      ...
      setTimeout(() => {
        try {
          ...
          this.queryResponse = inquirer.query(
            this.queryString,
            this.data.myDataSet
          );
  ...
  onError(event: any) {
    setTimeout(() => {
      this.transformationError = event;
    }, 666);
  }
  ...
  transformationDataSet(event: any) {
    ...
    if (this.selectedEntry === 'users') {
      ...
      this.data.transformations =
        this.selectionContents[this.selectedEntry].transformations;
      this.transformationsCopy = JSON.parse(
        JSON.stringify(this.data.transformations)
      );
    }
